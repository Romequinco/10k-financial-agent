...
> Requisitos: R11, R12, R13, R15 (y R10, R14) · Lee antes: [13_teoria_evaluacion_llms.md](13_teoria_evaluacion_llms.md) §9–§11,
> [21_skill_agente_salida_estructurada.md](21_skill_agente_salida_estructurada.md) §7 (`ejecutar()`), [25_skill_evaluadores.md](25_skill_evaluadores.md)
> §7–§8 y §11 (`evaluar()`, `puntuar()`, `resumen.json`), [24_skill_mejora_retrieval.md](24_skill_mejora_retrieval.md) §10 (escalera)
...
> docs 21–25 y 30–33; notas A1, A5, A6, B2, C1, C2, D3 y D12; decisiones D16–D21, D24 y D25 y contradicciones C01, C28 y C29 del mapa de cobertura.
...
`ejecutar()` ([21 §7](21_skill_agente_salida_estructurada.md)) y la agregación, `puntuar()` ([25 §8](25_skill_evaluadores.md)). Aquí va lo que falta
...
| Escalera de retrieval | R08, R11 | `resultados/retrieval/escalera.csv` | `metricas_retrieval.py` ([24 §8](24_skill_mejora_retrieval.md)) |
...
| --- | --- | --- | --- |
| USD/pregunta | `get_usage_metadata_callback()` alrededor de `invoke`; Σ tokens × precio buscado por prefijo de `model_name` (`usd()`, [21 §7](21_skill_agente_salida_estructurada.md)) | bucle, reintentos de R05, LLM dentro de una tool | juez ([25 §6](25_skill_evaluadores.md)) y reescritura aislada de la escalera ([24 §7](24_skill_mejora_retrieval.md)) |
...
- **Callback y no la suma de `messages`**: el callback también ve las llamadas al LLM fuera del bucle, y la diferencia queda en
  `tokens_fuera_del_bucle` [32 §3.6](32_repo_transformers_labs.md) (probado). `get_num_tokens()` no sirve: cuenta con el tokenizador de GPT-2
  [30 §3](30_clase_pistas_del_profesor.md) (venv).
...
- La latencia depende de la carga de OpenRouter. Por eso la repetición del baseline se hace **el mismo día** que la ejecución final (§4).
  ...
  segundo no es realista: se reporta lo que salga [13 §11](13_teoria_evaluacion_llms.md).

`evaluar()` llama a `calentar()` y a `top20_sistema()` ([25 §8](25_skill_evaluadores.md)), y `usd()` necesita `PRECIOS`. Faltan estas tres
piezas **(compila)**:
...
PRECIOS = cargar_precios()                                        # el que usa usd() dentro de ejecutar() (21 §7)
...
# from agente10k.config import SISTEMA, MODELO_ID, crear_modelo   -> 21 §2 y 25 §8
# from agente10k.metricas_retrieval import RUTA_CACHE             -> 24 §8
...
def top20_sistema(pregunta: str) -> list[dict]:
    ...
    if SISTEMA == "baseline":
        return miax_s1.buscar(pregunta, k=20)                     # ≡ buscar_v2(..., cfg=PASO0) [24 §3]
...
del profesor (D21, [21 §1](21_skill_agente_salida_estructurada.md)): las tools del notebook con su cuerpo tal cual (BPA redondeado incluido),
...
Pasos, antes del 17-sep: esa sesión empieza ejecutando cada baseline contra cinco preguntas duras [30 §1](30_clase_pistas_del_profesor.md).

1. Validar `golden/golden_propio.jsonl` y `golden/golden_huecos.jsonl` y hacer commit **antes** de ejecutar ([23 §1](23_skill_golden_set.md)).
...
3. Ejecutar. La etiqueta se pasa explícita: sin ella, el *stem* del fichero daría `baseline_golden_propio`, que `generar()` (§6) no busca
   ...
   lo calcula `evaluar()` en `rankings.jsonl`; la escalera completa se mide después ([24 §8](24_skill_mejora_retrieval.md)).
...
4. Revisar la `tasa_fallo` del resumen. Si hay errores de red o de clave (400/401 [30 §4](30_clase_pistas_del_profesor.md)), se repite **la
...
- A partir de aquí, `resultados/baseline_*` **no se sobrescribe**. Si cambian los evaluadores, se vuelve a puntuar con
  `puntuar("baseline_golden")` ([25 §8](25_skill_evaluadores.md)), con el juez desde la caché.
...
3. **Las repeticiones estiman el ruido y no se promedian** [13 §9](13_teoria_evaluacion_llms.md). En la tabla van la ejecución congelada del
...
# from agente10k.api import RESULTADOS, usd                           -> 25 §8 y 21 §7
# from agente10k.config import RUTA_PRECIOS, cargar_precios           -> §2
# from agente10k.evaluadores import enrutado, metricas_abstencion     -> 25 §4 y §7 (33 §3.9)
...
def aviso_recall(fila: dict, paso: str) -> str:
    """El recall@5 de R11 es el del paso de la escalera que usa el sistema (24 §1). Si no cuadra, algo cambió entre medidas."""
...
- **Siempre k/n.** Una familia vacía se muestra como "—" y no entra en la macro [25 §7](25_skill_evaluadores.md). Cada pregunta vale 5 pp y
  cada ancla, ≈ 7 pp [13 §9](13_teoria_evaluacion_llms.md).
- **Negrita por bloque.** El golden oficial (C28) va en un bloque aparte con su propia negrita: la tabla del enunciado es la del golden
  propio [enunciado · §5]. Una llamada menos solo es mejor con aciertos parecidos: eso se dice en la lectura [13 §11](13_teoria_evaluacion_llms.md).
...
def p_signo(g: int, p: int) -> float:
    """Test de signo exacto bilateral (= McNemar exacto) sobre las discordantes. Propuesta propia (13 §9): se informa, no se presume."""
    n = g + p
...
def fila_abstencion(nombre: str, golden: str, huecos: str) -> dict:
    """R14 en las dos direcciones (D11) y el punto riesgo-cobertura del sistema (33 §3.9)."""
...
def fila_coste(nombre: str, etq: str, precios: dict) -> dict:
    ...
    pred = [f for f in pred if "uso" in f]                                     # las que llegaron a ejecutarse, como en 25 §8
...
def generar() -> None:
    ...
    if faltan := [e for e in ("baseline_golden", "final_golden", "baseline_huecos", "final_huecos") if not existe(e)]:
        print(f"⚠️ Sin resumen.json para {', '.join(faltan)}: sus filas no salen (¿evaluar() sin etiqueta? 25 §8)")
...
| --- | --- | --- |
| `ganadas_perdidas` y `ruido` | Preguntas que el final gana y pierde, con su id y el p del test de signo; cuántas cambian al repetir sin tocar nada | "¿mejoró de verdad?": +2 netas pueden ser 2–0 o 6–4 [13 §9](13_teoria_evaluacion_llms.md). Con 5–1 sale p ≈ 0,22: se da la dirección, no se reclama significación (probado) |
...
| `enrutado` | P/R/F1 por herramienta, en total y por familia ([25 §4](25_skill_evaluadores.md)) | "¿Cómo enruta entre la exacta y la difusa?" [enunciado · §5] |
| `abstencion` | Abstención correcta en huecos e indebida en las 20, cobertura, alucinación, score con t = 0,75 y R05 | Honestidad (R14). Baseline y final como dos puntos de la curva riesgo-cobertura [33 §3.9](33_repo_generative_ai.md) |
| `coste` | Tokens por pregunta, tokens fuera del bucle, USD/acierto, contraste con OpenRouter y juez | "¿Qué costó?" El USD/acierto (propuesta de [32 §3.6](32_repo_transformers_labs.md)) lee el trade-off [slides RAG · p.40] |
| `causas` | Primera causa de cada fallo, por sistema y en las ciegas | Qué arreglar y, el 24, qué fallos son nuevos (§8) |
| `escalera` | [24 §10](24_skill_mejora_retrieval.md), tal cual | Qué aportó cada arreglo de retrieval |
...
- **Un cambio por fila**, medido contra la fila anterior con las mismas preguntas y en el mismo orden [24 §2](24_skill_mejora_retrieval.md).
...
- Lo que conviene comprobar y contar en k/n: BM25 sin reescritura, el filtro con el techo ya alto, preguntas perdidas con la reescritura y la
  distancia entre filtros oráculo y del LLM ([24 §12](24_skill_mejora_retrieval.md)); forzar `tool_choice` ([21 §11](21_skill_agente_salida_estructurada.md));
  `sin_repetir` ([22 §3](22_skill_guardrails_middleware_xbrl.md)); listar los huecos en el prompt (experimento de D22); un modelo más barato,
  solo como experimento [30 §2](30_clase_pistas_del_profesor.md).
- Un Δ dentro del ruido de §6 cuenta como "no movió la métrica". Se decide con razones generales, no mirando qué pregunta concreta cambió:
  eso sería iterar contra el golden ([13 §10](13_teoria_evaluacion_llms.md)).
...
- [ ] Plan B de red: datos del móvil, porque en Colab se vieron errores 400/401 que un alumno atribuyó a IPs bloqueadas
  [30 §4](30_clase_pistas_del_profesor.md). Plan C: el `demo.ipynb` de §10 en Colab.
...
- [ ] Si las ciegas llegan en otro formato (lista JSON, CSV o texto), un conversor a JSONL ya probado: convertir el fichero no es tocar
  código. ⚠️ Formato desconocido (duda 9 de [30 §8](30_clase_pistas_del_profesor.md)); lo que aguanta `evaluar()`, en [25 §11](25_skill_evaluadores.md).
...
4. **Síntomas de memoria** que hay que revisar con honestidad [13 §10](13_teoria_evaluacion_llms.md): casos del golden dentro de docstrings
...
5. Si las ciegas no traen ancla, no hay recall ("—"). Si no traen `herramienta_esperada`, (c) usa la lista por familia (`c_por_defecto`).
   Las dos cosas se dicen [25 §11](25_skill_evaluadores.md).
...
| 2 | Problema y diseño: las 4 tools y su coste (≈40 / ≈2.000 / hasta 34.751 tokens [01 §3]), diagrama del agente y acierto por familia (D14) | [01](01_requisitos_y_contratos.md), [21](21_skill_agente_salida_estructurada.md) |
...
| 5 | Retrieval: escalera en k/n, techos y % de filtros bien extraídos | [24 §10](24_skill_mejora_retrieval.md) |
| 6 | Enrutado exacta/difusa y guardrails: P/R/F1 por herramienta, la celda 23 antes y después, R05 con un mensaje real y abstención | `enrutado.md`, `abstencion.md`, [22 §10](22_skill_guardrails_middleware_xbrl.md) |
...
| 9 | Limitaciones y conclusión: n = 20, ruido, κ del juez, tolerancias y coste como cota superior | `validacion_juez.md` ([25 §9](25_skill_evaluadores.md)) |
| Anexos | A. Metodología (tolerancias, recall@5, acierto, juez); B. Tablas secundarias; C. Golden (reparto y huecos); D. Cómo reproducir (comandos de §3, §4 y §8) | §6, [23](23_skill_golden_set.md) |
...
- [ ] `requirements.txt` con los pines de la celda 2 [01 §4](01_requisitos_y_contratos.md), más `pandas` y `pyarrow` fijados. ⚠️ La versión
  está por decidir: la de Colab el día del ensayo; [32](32_repo_transformers_labs.md) se probó con pandas 2.3.3.
...
- [ ] Clave: `OPENROUTER_API_KEY` por entorno → Secrets de Colab → `getpass` ([21 §2](21_skill_agente_salida_estructurada.md)).
...
- [ ] Diez "ciegas simuladas", escritas por quien no escribió el golden y con **solo** los campos oficiales (una hueco y otra sin ancla),
  ejecutadas en el clon sin editar nada [25 §11](25_skill_evaluadores.md).
...
- [transcripcion_10sep · 00:08 (entrega), 00:26–00:28 (coste y latencia), 02:10–02:12 (400/401), 02:24 ("30 % menos de tokens"), 02:31
  (GitHub y presentarlo)]; [transcripcion_04sep · 00:36] (claves), vía [30](30_clase_pistas_del_profesor.md) §1–§4 y §8.
- [slides RAG · p.40, p.45, p.50] vía nota A5; [slides Tuning · p.54–55] y [slides agenda · p.12] vía nota A6 y [30 §7](30_clase_pistas_del_profesor.md).
- [32 §3.6](32_repo_transformers_labs.md) (callback, micro/macro, ganadas/perdidas); [33 §3.1, §3.6, §3.9](33_repo_generative_ai.md)
  (enrutado, coste por llamada, abstención); [31 §3.6](31_repo_genai_labs.md) (`responder()` con coste y latencia).
- generative-ai · `gemini/sample-apps/gemini-hallcheck/src/gemhall/eval.py` (artefactos por ejecución) y `tools/llmevalkit/README.md` (blind case), vía nota D12 y [13 §10](13_teoria_evaluacion_llms.md).
...
- Docs hermanos: [02 §8](02_datos_corpus_y_xbrl.md), [13 §9–§11](13_teoria_evaluacion_llms.md), [21 §2, §7, §10–§11](21_skill_agente_salida_estructurada.md),
  [22 §3, §10](22_skill_guardrails_middleware_xbrl.md), [23 §1](23_skill_golden_set.md), [24 §2–§3, §7–§10, §12](24_skill_mejora_retrieval.md),
  [25 §4, §7–§9, §11](25_skill_evaluadores.md).
