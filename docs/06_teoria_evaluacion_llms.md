...
> [04_teoria_rag_retrieval.md](04_teoria_rag_retrieval.md), [05_teoria_agentes_react_tools.md](05_teoria_agentes_react_tools.md)
> · Después: [12_skill_evaluadores.md](12_skill_evaluadores.md), [13_skill_medicion_informe_presentacion.md](13_skill_medicion_informe_presentacion.md)

> Fuentes: [enunciado · Datos básicos, §1, §4.5, §5, §7]; slides de RAG (p.40–50) y de Tuning (p.50–57); [transcripcion_04sep · 00:52];
> [transcripcion_10sep · 02:26–02:29]; transformers-labs `05-llm/` (vía [16](16_repo_transformers_labs.md) §3 y §5); generative-ai
> `gemini/evaluation/` y `hc/` (vía [17](17_repo_generative_ai.md) §3.1, §3.2, §3.7 y §3.9); decisiones D06–D19 del mapa de cobertura.

**v1.0 · 12-sep-2026.** Conceptos para diseñar los evaluadores y leer la tabla baseline frente a final. El código está en
[12](12_skill_evaluadores.md) y la tabla y el informe, en [13](13_skill_medicion_informe_presentacion.md). Abreviaturas de las citas:
...
- **ROUGE-L:** F sobre la subsecuencia común más larga (LCS), que **no es contigua** [05-6 · celda 5]. Palabras sueltas de un párrafo
  puntúan alto aunque la frase no exista, y el mismo par con error ×10 da ROUGE-L = 0,75 (probado, [16 §3.2](16_repo_transformers_labs.md)).
  El notebook usa β = P/R y su LCS recursiva revienta con textos de 1.200 palabras [16 §5](16_repo_transformers_labs.md).
- **BLEU:** brevity penalty × media geométrica de las precisiones de 1 a 4-gramas con *clipping* [05-7 · celdas 4, 12, 18]. La
  ...
  [16 §5](16_repo_transformers_labs.md)). Bien implementado, sigue premiando el solape léxico: castiga una paráfrasis correcta en español
...
- **Cobertura de 4-gramas contiguos:** la precisión de n-gramas de [05-7 · celda 13] con n = 4 y sin *clipping*
  ([16 §3.2](16_repo_transformers_labs.md)). Cambiar una palabra en una frase de 25 la deja en ≈ 0,82 (cálculo propio). Sirve para separar
...
- **Modo aislado frente a dentro del agente.** La tabla R11 lleva el aislado (la función de búsqueda con la pregunta); `recall_agente` (el
  ...
  [04](04_teoria_rag_retrieval.md) y [11](11_skill_mejora_retrieval.md).
...
(Ejemplo calculado con `metricas_trayectoria` de [17 §3.1](17_repo_generative_ai.md), probado.)
...
- **Argumentos.** Las métricas de parámetros de Vertex solo miran `tool_calls[0]` y dividen por la unión de claves
  ...
  0,04 % [16 §3.3](16_repo_transformers_labs.md)) y la confusión de año.
...
- **Groundedness frase a frase** (estilo FACTS): cada frase de la respuesta es `supported`, `unsupported`, `contradictory` o `no_rad` (no
  ...
  Resuelve C10 frente al SUPPORTS/REFUTES/NOT_ENOUGH_INFO de [15 §3.5](15_repo_genai_labs.md), que da una etiqueta por respuesta.
...
consenso [celda 21]) apenas aporta a `temperature=0`. Umbral de aceptación: propuesta propia en [12](12_skill_evaluadores.md).
...
- Con un 0,5 % relativo en el BPA, el diluido 6,08 pasaría por el básico 6,11 (0,49 %) [16 §3.3](16_repo_transformers_labs.md): por eso el BPA
...
- **Escala.** Se reescala por palabras de la unidad ("millones", "billion"); si el valor cuadra a ×10^±3/6/9, se etiqueta `error_escala`, que
  sigue siendo fallo. "Billones" (10¹²) se deja fuera a propósito: "60,9 billones" es un error de ×1.000 [09 §4](09_skill_guardrails_middleware_xbrl.md).
...
- Falta la mitad que pide el profesor: detectar cuándo "te ha dicho que no había respuesta cuando sí que había"
  ...
  "ninguna"). Sin la segunda, un agente que siempre dice "no está" parece honesto [17 §3.9](17_repo_generative_ai.md).
...
- **Ruido.** `temperature=0` hace la decodificación casi voraz, pero no garantiza el mismo resultado entre llamadas: ⚠️ no se ha comprobado
  ...
  [16 §3.6](16_repo_transformers_labs.md). Las repeticiones sirven para estimar el ruido, no para promediar la tabla.
...
- Síntomas de memoria del golden: docstrings o prompt con casos concretos del golden, reglas "si preguntan X, usa Y" para una pregunta, la
  lista de huecos en el prompt (D22), preguntas escritas con el vocabulario del chunk (inflan BM25 [17 §3.8](17_repo_generative_ai.md)) e
...
- **Coste:** tokens × precio de una tabla fechada, con el callback de uso, que también cuenta la reescritura y los reintentos de R05 (D16). El
  juez va aparte: no es coste del agente. "Coste por acierto" ayuda a leer el trade-off (propuesta de [16 §3.6](16_repo_transformers_labs.md)).
...
- transformers-labs · `05-llm/05-5-eval-classification.ipynb`, `05-6-eval-summarization.ipynb`, `05-7-eval-textgeneration.ipynb`,
  `05-8-eval-qna.ipynb`, consolidados en [16_repo_transformers_labs.md](16_repo_transformers_labs.md) §3.1–3.4, §3.6 y §5.
- generative-ai · `ET/evaluating_langgraph_agent.ipynb`, `ev/evaluate_with_your_python_code.ipynb`, `ET/evaluate_groundedness_with_custom_parsing.ipynb`,
  ...
  `tools/llmevalkit/README.md`, consolidados en [17_repo_generative_ai.md](17_repo_generative_ai.md) §3.1, §3.2, §3.7–3.9.
- [15_repo_genai_labs.md](15_repo_genai_labs.md) §3.5 (juez de tres etiquetas); [02_datos_corpus_y_xbrl.md](02_datos_corpus_y_xbrl.md) §3 y §5;
  [09_skill_guardrails_middleware_xbrl.md](09_skill_guardrails_middleware_xbrl.md) §4.
