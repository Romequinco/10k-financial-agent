...
> Requisitos: R03, R04 (montaje), R10, R13 · Lee antes: [12_teoria_agentes_react_tools.md](12_teoria_agentes_react_tools.md),
> [20_skill_herramientas_docstrings.md](20_skill_herramientas_docstrings.md), [31_repo_genai_labs.md](31_repo_genai_labs.md) §3.1, §3.3, §3.6 y §3.9

> Fuentes: `00_enunciado.md`, `01_requisitos_y_contratos.md`, notebook S1 (celdas 4, 6 y 19–29), transcripción del 10-sep, docs 30–33,
...
y R05 en [22](22_skill_guardrails_middleware_xbrl.md); herramientas en [20](20_skill_herramientas_docstrings.md); `evaluar()` y tablas en
[26](26_skill_medicion_informe_presentacion.md). Marcas: **(probado)** = ejecutado en el venv del stack con `GenericFakeChatModel`, sin red
...
| Middleware | ninguno | límites + `VerificadorXBRL` ([22](22_skill_guardrails_middleware_xbrl.md)) | `middleware.py` |
...
- **La instancia, nunca la cadena.** Con una cadena, `create_agent` hace `init_chat_model(model)` sin `temperature` [01 §5, fallo 9] (venv).
  ...
  `profile=None` (venv). Sirve también al reescritor de [24](24_skill_mejora_retrieval.md) y, por defecto, al juez de [25](25_skill_evaluadores.md).
- Nada de `openrouter:auto` ni `ModelFallbackMiddleware` en lo evaluado: si el modelo cambia, baseline frente a final no significa nada
  [notebook S1 · celda 6; 30 §3]. `AGENTE10K_MODELO` solo para experimentos; `ejecutar()` guarda el id en cada fila.
- `OPENROUTER_API_KEY` es la variable que lee langchain-openrouter 0.2.8 [31 §3.9](31_repo_genai_labs.md) (probado). Antes de cada push,
...
- **Por qué el validador:** en el contrato solo `respuesta` y `fuente` son obligatorios, así que el esquema **no obliga a citar**, aunque la
  celda 25 diga lo contrario [30 §5, matiz 4]. Con `ToolStrategy` el `ValueError` vuelve al modelo como `ToolMessage` "Error: Failed to parse
  ...
  exista lo mira el evaluador (a) de [25](25_skill_evaluadores.md).
...
(probado); el aviso de la celda 26 de que el esquema suelto "falla" no aplica con este stack [30 §5, matiz 3]. Juez y reescritor:
...
- Parte del `SYSTEM` de la celda 21 y añade el universo, el significado de `fuente` y la cita literal (D22). **No lista los huecos**: con la lista
  ...
  [20](20_skill_herramientas_docstrings.md)). Listarlos es un experimento aparte.
- Los 13 conceptos, la regla del revenue y el coste de cada tool van en los docstrings ([20](20_skill_herramientas_docstrings.md)). Nada de
  ejemplos del golden set ni de cifras sin verificar [31 §3.8](31_repo_genai_labs.md). Prompt estático: favorece la caché de prefijo ⚠️
  [33 §3.6](33_repo_generative_ai.md).
...
def construir_agente(modelo, tools, system_prompt, valor_xbrl=None, hechos=None):
    ...
    return create_agent(
        ...
        middleware=pila_middleware(verificador),                   # límites + R05 (22)
...
- **Middleware**, en este orden: `ToolCallLimitMiddleware(run_limit=8)`, `ToolCallLimitMiddleware(tool_name="read_section", run_limit=2)`,
  ...
  [22](22_skill_guardrails_middleware_xbrl.md)). El baseline va sin middleware: es el "antes" de R04.
...
evaluadores y la tabla R11, y es lo que llama `evaluar()` [26](26_skill_medicion_informe_presentacion.md).
...
# from agente10k.datos import valor_xbrl, hechos                        -> 22 §4
...
| Campo | Para qué | Nota |
| --- | --- | --- |
| `tool_calls`, `n_llamadas` | (c) y "llamadas/pregunta" (D12, D18) | Lista blanca: fuera la tool sintética. Las llamadas bloqueadas por un límite siguen contando [31 §3.2](31_repo_genai_labs.md) |
| `observaciones` | "vista" del evaluador (a) (D13) | Contenido de los `ToolMessage` de `search_filings` y `read_section` |
| `uso`, `usd` | Coste R11 (D16) | Callback por `model_name`: incluye reescritura y reintentos de R05. Con un modelo sin `model_name` no cuenta nada [32 §3.6](32_repo_transformers_labs.md) |
| `uso_mensajes`, `tokens_fuera_del_bucle` | Contraste | Suma de `usage_metadata` de `messages` [31 §3.6](31_repo_genai_labs.md) |
...
`get_num_tokens()` no se usa: `ChatOpenRouter` no lo redefine y cuenta con el tokenizador de GPT-2 [30 §3] (venv).
...
Versión autónoma, con tool calling nativo: [31 §3.3](31_repo_genai_labs.md) (probado). Al historial va el `AIMessage` entero, no su texto
...
`agente.get_state(cfg).values["messages"]`. Ojo: `pretty_trace` (celda 28) rotula `cita:` pero imprime el `chunk_id` [30 §5, matiz 8].
...
Sin el arnés, el baseline tendría el `recursion_limit` por defecto de langgraph 1.2.11, 10007 pasos [30 §5, matiz 1] (venv).
...
2025 en el mismo `AIMessage` (paralelas; no hace falta ampliar la firma [33 §3.3](33_repo_generative_ai.md)), luego `search_filings(<consulta en
inglés>, ticker="MSFT", fiscal_year=2025, item="7", k=5)` y `search_filings(<misma consulta>, ticker="MSFT", fiscal_year=2024, item="7", k=5)`,
una por ejercicio ([20 §5](20_skill_herramientas_docstrings.md)). Respuesta: `fuente="ambas"`, `cifra=281724000000`, `ejercicio=2025`,
`cifra_base=245122000000`, `ejercicio_base=2024`, `unidad="USD"`, cita literal del Item 7 y la variación (+14,9 %) solo en la prosa
[perfil_dataset · xbrl_facts.parquet]. El formato del golden para esto está en [23](23_skill_golden_set.md) (D10).
...
Además, en la primera ejecución del golden: cómo reacciona Gemini al mensaje `[VERIFICADOR XBRL]` ([22](22_skill_guardrails_middleware_xbrl.md)).
...
la primera llamada al modelo: el código probado está en [33 §3.3](33_repo_generative_ai.md). Solo para preguntas clasificadas como numéricas,
...
Con un modelo falso: `GenericFakeChatModel` con `bind_tools` que devuelve `self` (patrón de [31 §3](31_repo_genai_labs.md)).
...
Probado así (con dos tools de juguete y el `VerificadorXBRL` de [22](22_skill_guardrails_middleware_xbrl.md)): el test anterior; el *fallback*
...
con su `tool_call_id`. Prueba de extremo a extremo: `evaluar()` con 2 preguntas en un clon limpio [26](26_skill_medicion_informe_presentacion.md).
...
6. Una cifra escalada: `cifra=60922` con `unidad="USD"` es un error ×10⁶ (`error_escala`); lo vigila R05 ([22](22_skill_guardrails_middleware_xbrl.md)).
...
- [transcripcion_10sep · 02:14, 02:17–02:24] y [transcripcion_04sep · 00:36], vía [30](30_clase_pistas_del_profesor.md) §2, §3 y §5.
- [31 §3.1, §3.3, §3.6, §3.8, §3.9](31_repo_genai_labs.md); [32 §3.6](32_repo_transformers_labs.md); [33 §3.3, §3.4, §3.6](33_repo_generative_ai.md).
