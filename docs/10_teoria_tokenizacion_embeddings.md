...
> [02_datos_corpus_y_xbrl.md](02_datos_corpus_y_xbrl.md) §3 · Después: [11_teoria_rag_retrieval.md](11_teoria_rag_retrieval.md),
> [24_skill_mejora_retrieval.md](24_skill_mejora_retrieval.md), [26_skill_medicion_informe_presentacion.md](26_skill_medicion_informe_presentacion.md)

> Fuentes: [transcripcion_04sep], [transcripcion_05sep], [slides NLP_S1] y [slides NLP_S2] (vía nota A34 y [30 §7](30_clase_pistas_del_profesor.md));
> [slides RAG · p.4, p.20, p.22, p.27–28, p.44]; transformers-labs `01-embeddings-tokenizers/` (vía [32 §3.5 y §3.7](32_repo_transformers_labs.md));
> generative-ai `embeddings/` (vía [33 §3.7](33_repo_generative_ai.md)); [api_stack · rank_bm25.BM25Okapi, AIMessage];
...
complementa al denso y por qué el coste lo cuenta el proveedor. La receta está en [24](24_skill_mejora_retrieval.md) y la medición del
coste en [26](26_skill_medicion_informe_presentacion.md). Abreviaturas: `01-N` = `transformers-labs · 01-embeddings-tokenizers/01-N-*.ipynb`;
...
| --- | --- | --- | --- |
| **BPE** | Fusiona una y otra vez el par más frecuente (`aaabdaaabac` → `XdXac`) | GPT-2, RoBERTa; `tiktoken` (OpenAI) | `tiktoken` solo aproxima la factura de Gemini [32 §3.7](32_repo_transformers_labs.md) |
...
- **En nuestro corpus** (datos; `BM25Okapi` sobre los 1.749 chunks con el tokenizer `[a-z0-9]+` de D09): longitud media de 296 tokens,
  ...
  ([24 §7](24_skill_mejora_retrieval.md)). Una lista de stopwords es una variante que se mide (C19).
...
```
  ...
  5. **IDF local**: construir BM25 solo con los candidatos cambia los IDF; se construye una vez (IDF global) y se puntúa con
     `get_batch_scores(tokens, ids)` (D09, [24 §6](24_skill_mejora_retrieval.md)).
...
- **Embeddings de frase, "dos torres" o bi-encoder**: Sentence-BERT (2019) [slides NLP_S1 · p.34]. Consulta y documento se codifican por
  ...
  es el dilema del chunking ([11 §2](11_teoria_rag_retrieval.md)).
...
- **Similitud.** L2, coseno o producto interno: "cualquiera de las tres te da un número" [transcripcion_05sep · 01:02]. Con vectores
  ...
  pierde el sentido [32 §3.5](32_repo_transformers_labs.md).
...
- **Multilingüe frente a solo inglés.** Lo primero de la ficha de un modelo es la dimensión, si es multilingüe ("house" ≈ "casa"), si es
  ...
  escalera usa la pregunta en español y el paso 3, la reescrita en inglés [24 §2](24_skill_mejora_retrieval.md). Un modelo multilingüe (el
  `distiluse-base-multilingual-cased-v1` de 512 d de 01-6 [32 §3.5](32_repo_transformers_labs.md)) obliga a reindexar y puede recuperar
...
- **Límite de entrada y truncado** ⚠️. Todo modelo de embeddings tiene un máximo de tokens de entrada: 2.048 en text-embedding-005 y en
  ...
  [32 §3.7](32_repo_transformers_labs.md); hay que correrla en un entorno con sentence-transformers antes de fiarse del paso 0.
...
- Aquí hay 1.749 vectores de 384 dimensiones en float32, unos 2,7 MB, y `buscar()` ordena el índice entero en cada consulta
  ...
  puntuaciones alineado por posición, que es lo que necesita la fusión con BM25 [32 §3.5](32_repo_transformers_labs.md). `IndexFlatIP` se
  queda.
...
3. **R08 · BM25 tal como fija D09**: tokenizer `[a-z0-9]+`, `k1=1.5, b=0.75, ε=0.25` documentados en el informe, IDF global, consulta como
   ...
   a ojo ([24 §11](24_skill_mejora_retrieval.md)).
4. **R08 · Prefijo BGE solo en la consulta.** Medir con y sin prefijo es un experimento gratis que enseña un fallo silencioso en la
   presentación (el heatmap `np.inner(E, E)` de [32 §3.5](32_repo_transformers_labs.md)).
5. **R07/R08 · Antes de medir, comprobad el truncado y las anclas.** Si alguna ancla cae en la cola de uno de los 6 chunks largos, el
   denso no puede verla; y `anclas_no_indexables` debe ser 0 (D08, [24 §2](24_skill_mejora_retrieval.md)).
...
| BM25 con una consulta `str` | "Todo ceros sin error" [33 §3.7](33_repo_generative_ai.md) (corpus de juguete; ya matizado) y, antes de corregirlo, [24 §12](24_skill_mejora_retrieval.md) | Con el corpus real, ranking basura distinto de cero (datos) | Nunca pasar un `str` |
...
- Clase: [transcripcion_04sep · 00:49, 01:05–01:24, 01:37, 02:00, 02:13–03:08, 03:55–04:00]; [transcripcion_05sep · 00:15–01:02, 01:59,
  ...
  [30 §7](30_clase_pistas_del_profesor.md).
- [slides RAG · p.4, p.20, p.27–28, p.44], vía nota A5.
- transformers-labs: 01-2 · celda 9; 01-6 · celdas 4, 6, 8, 10, vía nota C1 y [32 §3.5 y §3.7](32_repo_transformers_labs.md).
- generative-ai: `task-type` · celdas 5, 18–19, 29, 46, 54, vía nota D12 y [33 §3.7](33_repo_generative_ai.md).
