...
> [14_clase_pistas_del_profesor.md](14_clase_pistas_del_profesor.md) §3 · Después: [07_skill_herramientas_docstrings.md](07_skill_herramientas_docstrings.md),
> [08_skill_agente_salida_estructurada.md](08_skill_agente_salida_estructurada.md), [09_skill_guardrails_middleware_xbrl.md](09_skill_guardrails_middleware_xbrl.md)

> Fuentes: [notebook S1 · celdas 0, 9–11, 16–29]; [transcripcion_10sep · 00:05–02:24] vía [14](14_clase_pistas_del_profesor.md); genai-labs
> `01-3-react.ipynb` vía [15 §3–§4](15_repo_genai_labs.md) y nota B1; generative-ai `fc/`, `adp/` y `controlled-generation/` vía
> [17 §3.3–§3.6, §4–§5](17_repo_generative_ai.md) y nota D4; [slides RAG · p.9, p.14–15, p.36–40] vía nota A5; [api_stack · create_agent, middleware,
...
qué el docstring es la interfaz. Sin recetas: el código está en 07–09. Marcas: **(probado)** = ejecutado en el venv del stack con
...
contaría como fallo [17 §3.6; enunciado · §1]; además, el dato puede quedar en medio del contexto (*lost in the middle*) [slides RAG · p.37–38].
...
lo que va detrás de `Args:` se pierde (venv; plantilla y pruebas en [07 §3](07_skill_herramientas_docstrings.md)). Lo que llega al proveedor
...
- **Paralelas.** Un solo `AIMessage` puede traer varias `tool_calls`; `create_agent` las ejecuta y añade un `ToolMessage` por `id` (probado). En una
  comparativa, los dos `get_xbrl_fact` en el mismo turno ahorran una vuelta de modelo sin ampliar la firma [17 §3.3; generative-ai ·
  ...
  si Gemini vía OpenRouter las emite (prueba de humo, [08 §10](08_skill_agente_salida_estructurada.md)).
- **AUTO / ANY / NONE.** En AUTO el modelo decide y puede contestar en prosa sin llamar; ANY obliga a llamar; con NONE contesta con lo aprendido
  ...
  27, 30, 43, vía 17 §3.3]. En `ChatOpenRouter.bind_tools(tool_choice=…)`: `"auto"`, `"none"`, `"any"` (se traduce a `"required"`), el nombre de una
  ...
  primera llamada (probado, [17 §3.3]). ⚠️ Sin verificar que OpenRouter y Gemini lo respeten: es un experimento, no el defecto
  ([08 §11](08_skill_agente_salida_estructurada.md)).
...
texto es *prompt*: dice qué hacer después [17 §3.3]. `ToolErrorMiddleware(on_error=…)` convierte excepciones en `ToolMessage` solo si se configura
...
`tool_calls` [15 §3.3]. El *Thought* queda dentro del modelo (texto o razonamiento interno ⚠️); lo que se ve y se evalúa es la trayectoria de
acciones.
...
def bucle_react(modelo_con_tools, por_nombre: dict, mensajes: list, max_vueltas: int = 8):
    """Esqueleto del bucle de la celda 21. Versión probada, con captura de errores: 15 §3.3 y 08 §8."""
    ...
    raise RuntimeError("Límite de vueltas agotado")       # genai-labs devolvía None en silencio [15 §3.1]
```
...
herramienta no deje claro que el dato no existe; el límite es la red de seguridad [17 §3.5].
...
todo lo observado antes [notebook S1 · celda 21; genai-labs · 01-3-react.ipynb · celda 7; 17 §3.6]. Si el agente hace 5 llamadas al modelo y
...
([07 §3](07_skill_herramientas_docstrings.md)). La latencia también crece con el contexto [transcripcion_10sep · 00:28]. Por eso el enrutado barato
...
- **`recursion_limit`.** Cuenta *supersteps* del grafo, no llamadas. El valor por defecto de langgraph 1.2.11 es **10007** (variable de entorno
  ...
  `ModelCallLimitMiddleware` y el arnés usa 100 (V2, [09 §2](09_skill_guardrails_middleware_xbrl.md)).
...
- **Rastro en la trayectoria** (probado, [08 §4](08_skill_agente_salida_estructurada.md)): un `AIMessage` con la tool call `RespuestaFinanciera` y un
  ...
  cuenta contra `ToolCallLimitMiddleware` [09 §2]; y `structured_response` **puede ser `None`** si un límite corta antes, así que siempre hay *fallback*.
- **Autocorrección de formato.** Con `handle_errors=True`, un `ValueError` del validador vuelve al modelo como "Error: Failed to parse structured output
  for tool 'RespuestaFinanciera': …" y el modelo reintenta (probado, [17 §3.4]). Sirve para exigir `cita` cuando `fuente` es texto: el esquema del
  contrato no obliga a citar [notebook S1 · celda 25; 14 §5], y los campos opcionales se rellenan "de memoria" si nada lo impide
  [generative-ai · controlled-generation/intro_controlled_generation.ipynb · celda 20, vía 17 §3.4].
...
| `wrap_tool_call` | Envuelve cada ejecución; recibe `request.tool_call` y `request.state` | `ToolMessage` o `Command` | `sin_repetir` (opcional, [09 §3](09_skill_guardrails_middleware_xbrl.md)) |
...
- **Saltos.** `@hook_config(can_jump_to=["tools", "model", "end"])` en `before_model`/`after_model` [api_stack · hook_config]. Un `after_model` que
  ...
  `structured_response` (V3, probado): es el mecanismo de R05 ([09 §5](09_skill_guardrails_middleware_xbrl.md)).
- **Límites de serie** [api_stack · ToolCallLimitMiddleware, ModelCallLimitMiddleware]: `ToolCallLimitMiddleware(*, tool_name=None, thread_limit=None,
  ...
  `structured_response=None` (probado, [17 §3.5]). De ahí la pila de D04: `ToolCallLimit(8)`, `ToolCallLimit(read_section, 2)`, `ModelCallLimit(12,
...
- **De serie, pero fuera de lo evaluado:** `ModelFallbackMiddleware` cambia el modelo evaluado y reintenta con los otros en cada fallo, sin
  recordar nada [14 §3]; `ModelRetryMiddleware` duplicaría el `max_retries=2` de `ChatOpenRouter` [09 §8]; `SummarizationMiddleware` sustituye el
  historial por un resumen y perdería el texto literal de las citas [17 §3.6]; `LLMToolSelectorMiddleware` es otra llamada al LLM para elegir entre
...
P/R/F1 por herramienta y la matriz familia × herramienta ([07 §9](07_skill_herramientas_docstrings.md), [12 §4](12_skill_evaluadores.md)). Un router
explícito previo (razonar antes de clasificar y un destino "Unsupported") [generative-ai · adp/semantic-router.ipynb · celdas 18, 21, vía 17 §3.3]
...
nota A1]. Basta con el plan en el prompt ([08 §5](08_skill_agente_salida_estructurada.md)) y llamadas en paralelo; un planner multiagente es mucho
más lento [generative-ai · adp/README.md · l. 88, vía 17 §5], y un agente por documento serían 48 [slides RAG · p.40]. Trampa: el split 10:1 de NVDA;
...
(V3, D15). El anti-patrón es el crítico→editor sin contador [17 §3.5]. En retrieval: reintentar sin `item` si no hay resultados [slides RAG · p.36],
medido aparte [11 §5](11_skill_mejora_retrieval.md).
...
estaba en el prompt y la función no lo comprobaba [17 §3.5]. Capas: entrada (fuera de corpus: no se bloquea, lo resuelven tools y prompt,
[09 §8](09_skill_guardrails_middleware_xbrl.md)); herramienta (validación y errores como texto); salida (esquema, validador y R05); bucle (límites).

**Principios de diseño de tools** (detalle en [07 §2](07_skill_herramientas_docstrings.md)): devuelven texto, "o algo asimilable como texto"
...
[transcripcion_10sep · 02:12]; dicen "no está" con claridad y nunca con un 0 [17 §3.3]; declaran su coste; y no esconden llamadas a otras tools del
contrato, que la trayectoria no vería [14 §3].
...
| `MessageGraph` | Deprecado en v1.0, se quita en v2.0 [17 §4] | `create_agent` |
...
[notebook S1 · celda 3]; el resto sale de [15 §4](15_repo_genai_labs.md) y [17 §4](17_repo_generative_ai.md).
...
- **R03:** `ToolStrategy(schema=RespuestaFinanciera)` explícito, *fallback* cuando `structured_response` es `None`, validador para exigir la cita
  ([08 §3–§4](08_skill_agente_salida_estructurada.md)).
- **R04:** `continue` no basta; `ModelCallLimit` corta y `recursion_limit=100` protege también al baseline ([09 §2](09_skill_guardrails_middleware_xbrl.md)).
- **R05:** un `after_model` con `jump_to="model"`, determinista y con un reintento ([09 §5](09_skill_guardrails_middleware_xbrl.md)).
...
- [transcripcion_10sep · 00:05, 00:28, 00:31–00:49, 01:24, 01:42–01:47, 02:01, 02:12–02:24], vía [14 §2–§3](14_clase_pistas_del_profesor.md).
- genai-labs `01-prompting/01-3-react.ipynb` (celdas 5, 7, 26–27) vía [15 §3.1–§3.3, §4](15_repo_genai_labs.md) y nota B1.
- generative-ai `fc/forced_function_calling.ipynb`, `fc/parallel_function_calling.ipynb`, `adp/semantic-router.ipynb`, `adp/README.md` y
  `controlled-generation/` vía [17 §3.3–§3.6, §4–§5](17_repo_generative_ai.md) y nota D4.
...
- Docs hermanos: [07](07_skill_herramientas_docstrings.md), [08](08_skill_agente_salida_estructurada.md), [09](09_skill_guardrails_middleware_xbrl.md),
  [11 §5](11_skill_mejora_retrieval.md), [12 §4](12_skill_evaluadores.md), [02 §4](02_datos_corpus_y_xbrl.md).
